...
// DecodeJSONOld if the type is known and decode error is unexpected. Intended to be paired with Inspect
func DecodeJSONOld[T any](r io.Reader) T {
	v := new(T)
	_ = json.NewDecoder(r).Decode(v)
	return *v
}

// DecodeYAMLOld if the type is known and decode error is unexpected. Intended to be paired with Inspect
func DecodeYAMLOld[T any](r io.Reader) T {
	v := new(T)
...
func DecodeBundle(r io.Reader) BundleOld {
	bun := NewBundleOld()
